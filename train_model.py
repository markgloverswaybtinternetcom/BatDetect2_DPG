import argparse, json, warnings, numpy, torch, datetime, os, glob, copy, torchaudio, librosa, traceback, colorama
...
class AudioLoader(torch.utils.data.Dataset):
    def __init__(self, data_anns_ip, params, dataset_name=None, is_train=False):
        ...
        for ii in range(len(data_anns_ip)):
            ...
            for ii, aa in enumerate(dd["annotation"]):  #### list indices must be integers or slices, not str ####
                ...
                compositeClass = aa["class"] + "-" + aa["event"]
                if compositeClass in self.params["class_names"]:
                    aa["class_id"] = self.params["class_names"].index(compositeClass)
                else:
                    print(colorama.Fore.RED + f"AudioLoader __init__ {compositeClass=} NOT FOUND" + colorama.Fore.RESET)
                    aa["class_id"] = -1
    ...
    def __getitem__(self, index):
        try:
            ...
            print(colorama.Back.RED + f"[ERROR] Failed to load index {index=}" + colorama.Back.RESET)
            traceback.print_exc()
            raise
    def __len__(self):
...
def train(model, epoch, data_loader, optimizer, scheduler, params):
    ...
    for batch_idx, inputs in enumerate(data_loader):
        try:
            ...
            print(colorama.Back.ORANGE + f"[WARNING] Skipping batch {batch_idx}: {e}" + colorama.Back.RESET)
            continue
